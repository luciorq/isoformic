...
vignette: |
  %\VignetteIndexEntry{isoformic-lncrna}
...
```{r}
PE1_DETs_final <- path_package("isoformic", "extdata", "PE1_DETs_final.rds") |>
  readr::read_rds()

tx_to_gene <- path_package("isoformic", "extdata", "tx_to_gene.rds") |>
  readr::read_rds()
```
...
```{r}
PE1_lncRNAs <- PE1_DETs_final |>
  filter(
    transcript_type == "lncRNA" & abs(log2FC) > 0.5 & pvalue < 0.05 & transcript_name %in% lnc_for_analysis
  )

head(PE1_lncRNAs)
```

So our first goal will be to get every transcript that is located around our lncRNAs. TAD lengths vary widely but over 80% of all TADs fall below 2 megabases, so here we will use 1 megabase to each side as a cut for possible cis-acting lncRNA.
...
Now we use the biomaRt select function to select the transcript start, transcript end transcript strand and chromosome columns for our selected lncRNA. You can reduce even more this list if you have a smaller number of lncRNAs you want to analyze. Reducing this number is suitable for running the transcript correlation later on. The time it takes for it will increase exponentially base don the amount of lncRNA you have here.
...
barplot(number_overlaps,
  names.arg = lnc_names$transcript_name,
...
```{r}
cor_lnc_targets_final <- cor_lnc_targets_res |>
  filter(row != column) |>
...
```{r}
cor_lnc_targets_final_filt <- cor_lnc_targets_final_filt |>
  left_join(tx_to_gene, by = c("row" = "transcript_name")) |>
  dplyr::select(1:5)

cor_lnc_targets_final_filt <- cor_lnc_targets_final_filt |>
  left_join(tx_to_gene, by = c("column" = "transcript_name")) |>
  dplyr::select(1:6)
...
```{r}
correlations_and_ranges <- joined_all |>
  filter(transcript_id_row %in% all_transcripts_onranges$tx_name & seqnames.x == seqnames.y) |>
...
What we will do here is quite direct, we will use the correlation value as the lncRNA-mRNA pairs to enrich and with that we will have Normalized Enrichment Scores which vary depending of the expression direction of the lncRNA and of it`s target.
