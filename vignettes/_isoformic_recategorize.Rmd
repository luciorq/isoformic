...
vignette: |
  %\VignetteIndexEntry{isoformic-recategorize}
...
The last issue we try to explore with `isoformic` is a re-classification of transcripts based on their modifications compared to the *canonical transcript*,
the one that is used as the "gene" reference,
and the quantification of this modifications in a gene-group basis. 
...
```{r}
download_mane_reference()
```


First we do the formatting of the MANE and the GENCODE GFF using the `format_gff()` function.

```{r}
#' Format MANE annotation
format_gff <- function(MANE_gtf_pathway, gencode_gff_pathway) {
  MANE_GRCh38_v1_2_ensembl_genomic <- readr::read_table(
    MANE_gtf_pathway,
    col_names = FALSE
  )
  tx_transcripts <- MANE_GRCh38_v1_2_ensembl_genomic$X12
  transcripts_MANE <- tx_transcripts[stringr::str_detect(tx_transcripts, "ENST")]
  transcripts_MANE_f <- gsub('"', "", transcripts_MANE)
  transcripts_MANE_f <- unique(
    gsub(";", "", transcripts_MANE_f)
  )
  mane_transcripts <- tx_to_gene |>
    dplyr::filter(transcript_id %in% transcripts_MANE_f)
  gencode_v34_annotation <- readr::read_delim(
    gencode_gff_pathway,
    delim = "\t", escape_double = FALSE,
    col_names = FALSE, trim_ws = TRUE, skip = 7
  )
  gencode_v34_annotation_tomod <- gencode_v34_annotation
  gencode_v34_annotation_tomod[1:12] <- gencode_v34_annotation_tomod$X9 |>
    stringr::str_split_fixed(";", 12)
  gff_v34 <- cbind(
    gencode_v34_annotation,
    gencode_v34_annotation_tomod
  )
  names(gff_v34) <- c(
    "chr", "HAVANA", "type", "left_pos", "right_pos", "exclude_1",
    "strand", "exclude_2", "etc", "ID", "V1", "V2", "V3", "V4",
    "V5", "V6", "V7", "V8", "V9", "V10", "V11"
  )
  gff_v34_filt <- gff_v34 |>
    dplyr::select(3:5, 7, 10)
  gff_v34_filt$ID <- stringr::str_sub(
    gff_v34_filt$ID,
    start = 4
  )
  gff_v34_filt$ID <- stringr::str_replace(
    gff_v34_filt$ID,
    "exon:",
    ""
  )
  ...
  df_split_notgene_dic <- df_split_notgene |>
    dplyr::left_join(tx_to_gene, by = c("tx_id" = "transcript_id"))
  df_split_notgene_dic <- df_split_notgene_dic |>
    dplyr::select(1:6, 9)
  df_with_all <- rbind(df_split_gene_dic, df_split_notgene_dic)
  df_transcripts_only <- df_with_all |>
    dplyr::filter(type == "transcript")
  MANE_dic <- mane_transcripts |>
    dplyr::left_join(df_transcripts_only, by = c("transcript_id" = "tx_id"))
  mane_filt <- MANE_dic |>
    dplyr::select(gene_name.x, left_pos, right_pos)
...
result_format <- format_gff(
  MANE_gtf_pathway =
    "MANE.GRCh38.v1.2.ensembl_genomic.gtf",
  gencode_gff_pathway =
    "gencode.v34.annotation.gff3"
)
```
...
With those two tables we can then run the isoformic_categorize function which takes a vector of genes as input and returns the classification of all the transcripts of that gene. if the gene you are inputting is *not* a MANE gene the function will *not* work since there will be no reference to compare.
...
isoformic_categorize <- function(gene_vector, df_with_all, mane_filt) {
  ...
  for (selected_gene in gene_vector) {
    ...
    exon_counts <- gtf_df |>
      ...
      dplyr::summarise(max(exon_count))
    names(exon_counts) <- c("tx_id", "max_exon")

    gtf_df_counts <- gtf_df |>
      dplyr::left_join(exon_counts, by = c("tx_id" = "tx_id"))

    transcripts_with_types <- gtf_df_counts |>
      dplyr::filter(type == "transcript") |>
      dplyr::mutate(category = dplyr::case_when(
        (
          strand == "+" & left_pos != default_left &
            right_pos == default_right &
            max_exon == default_exon_count
        ) ~ "aTSS",
...
genes_recategorized <- isoformic_categorize(
  gene_vector = vector_with_genes,
  mane_filt = result_format$mane_filt,
  df_with_all = result_format$df_all
)
```

We can use the data frame produced for the profile plot to visualize the counts of the different types of transcripts between case and control and also inside the pathways that were returned on enrichment.
A good way to visualize this would be any graph that can compute well proportions, a barplot would do, but to be able to visualize groups and subgroups we opted for a treemap type graph.
...
genes_recategorized_dic <- genes_recategorized |>
  dplyr::left_join(
    tx_to_tx,
...
```{r}
categories_vector <- c(
  "aTSS", "aTES", "intron_retained", "exon_skipping", "aTSS aTES exon_skipping",
...
cat_colors <- c(
  "#b15928", "#33a02c", "grey", "#a6cee3",
  "#ffff99", "#1f78b4", "#ff7f00", "#fdbf6f",
  "purple", "#e31a1c", "#cab2d6", "#b2df8a", "#fb9a99"
)
...
It is possible to also use this information with the pathway results.
Here an example using the df_enrichment output from the functional transcript enrichment for the Angiogenesis pathway genes.

First we isoformic recategorize those transcripts
```{r}
enrichment_angiogenesis_productive <- enrichment_df |>
  filter(pathway == "HALLMARK_ANGIOGENESIS" & experiment == "protein_coding")

genes_angiogenesis_prod <- mane_filt |>
  filter(gene_name %in% unlist(enrichment_angiogenesis_productive$leadingEdge))

angiogenesis_recategorized <- isoformic_categorize(
  gene_vector = genes_angiogenesis_prod$gene_name,
  mane_filt = result_format$mane_filt,
  df_with_all = result_format$df_all
)
```
...
```{r}
angiogenesis_recategorized_dic <- angiogenesis_recategorized |>
  left_join(
    tx_to_tx,
    by = c("tx_id" = "transcript_id")
  )

angiogenesis_recategorized_dic <- angiogenesis_recategorized_dic |>
  left_join(profile_data_df, by = c("transcript_name" = "genename"))
```
